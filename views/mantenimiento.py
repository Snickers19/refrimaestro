...
def MantenimientoView(page: ft.Page):
    ...
    def generar_resumen(e):
        ...
        for cb in checkboxes:
            if cb.value:
                ...
                desglose += f"  • {s['nombre']}: {format_clp(s['precioBase'])}\n"
        ...
        if not hay_seleccion:
            ...
            desglose += "* Valores referenciales. El total final depende de la visita técnica."

        resumen_text.value = desglose
        resumen_container.visible = True
        page.update()

    # ── Checkboxes con label nativo ──────────────────────────────────────────
    checkbox_controls = []
    for s in servicios:
        cb = ft.Checkbox(
            label=f"{s['nombre']}  ({s['precioTxt']})",
            label_style=ft.TextStyle(color=TEXT_PRIMARY, size=14),
            data=s,
            on_change=calcular_total,
            fill_color={
                ft.ControlState.SELECTED: COFFEE_COLOR,
                ft.ControlState.DEFAULT:  "#2a2d36",
            },
    ...
    pricing_table = ft.DataTable(
        ...
        rows=[
            ft.DataRow(cells=[
                ft.DataCell(ft.Text("Consulta base",    color=TEXT_SECONDARY)),
                ft.DataCell(ft.Text("Evaluación y diagnóstico en domicilio.",       color=TEXT_SECONDARY)),
                ft.DataCell(ft.Text("$15.000", color=COFFEE_COLOR, weight=ft.FontWeight.BOLD)),
            ]),
            ft.DataRow(cells=[
                ft.DataCell(ft.Text("Cambio de filtro", color=TEXT_SECONDARY)),
                ft.DataCell(ft.Text("Cambio de pieza filtro para refrigeración",    color=TEXT_SECONDARY)),
                ft.DataCell(ft.Text("$35.000", color=COFFEE_COLOR, weight=ft.FontWeight.BOLD)),
            ]),
            ft.DataRow(cells=[
                ft.DataCell(ft.Text("Recarga de gas",   color=TEXT_SECONDARY)),
                ft.DataCell(ft.Text("Detección/reparación de fugas y carga",        color=TEXT_SECONDARY)),
                ft.DataCell(ft.Text("$45.000", color=COFFEE_COLOR, weight=ft.FontWeight.BOLD)),
            ]),
            ft.DataRow(cells=[
                ft.DataCell(ft.Text("Cambio de aceite", color=TEXT_SECONDARY)),
                ft.DataCell(ft.Text("Revisión de compresor y cambio de aceite",     color=TEXT_SECONDARY)),
                ft.DataCell(ft.Text("$30.000", color=COFFEE_COLOR, weight=ft.FontWeight.BOLD)),
            ]),
        ],
        border=ft.border.all(1, "#2a2d36"),
        border_radius=12,
    ...
    return ft.Column(
            ...
            ft.Container(
                content=ft.Column(
                    [
                        ...
                        ft.Text(
                            "Transparencia en nuestros precios. Conoce nuestras tarifas de revisión y reparación.",
                            size=16,
                        ...
                        ft.Container(
                            content=ft.Column(
                                [
                                    ...
                                    ft.Text(
                                        "Selecciona los servicios para obtener un presupuesto aproximado.",
                                        color=TEXT_SECONDARY,
                                    ),
        ...
        expand=True,          # ← fix principal: activa scroll y visibilidad completa
    )